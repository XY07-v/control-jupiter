...
<style>
    ...
    .card-mini { background: white; border-radius: 12px; padding: 15px; border: 1px solid #eee; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
    
    /* UNIFICACIÓN DE BOTONES */
    .btn-g { 
        height: 42px; width: 100%; border-radius: 8px; border: none; 
        font-size: 14px; font-weight: 600; cursor: pointer; 
        display: flex; align-items: center; justify-content: center; 
        transition: 0.2s; text-decoration: none; margin-top: 8px; box-sizing: border-box;
    }
    .action-bar .btn-g { width: auto; padding: 0 20px; margin-top: 0; }
    ...
    .modal-content { background: white; margin: 5% auto; width: 90%; max-width: 500px; border-radius: 15px; padding: 25px; max-height: 80vh; overflow-y: auto; }
...
def index():
    ...
    <body>
        <header>
            <div style="font-weight: 800; color: var(--nestle-blue);">Nestlé BI Admin</div>
        ...
        <div class="container">
            ...
            <input type="text" id="search" placeholder="Filtrar..." onkeyup="filtrar()">
        ...
        <div id="m_csv" class="modal"><div class="modal-content">
            ...
            <button class="btn-g btn-primary" onclick="subirCSV()">Procesar</button>
            <button class="btn-g btn-outline" onclick="closeModal()">Cerrar</button>
        </div></div>
        ...
        <script>
            ...
            async function cargar(tipo) {{
                ...
                let html = '';
                data.forEach(d => {{
                    if(tipo === 'validaciones') html += `<div class="card-mini"><b>${{d.pv}}</b><br><small>${{d.n_documento}}</small><button class="btn-g btn-primary" onclick="verDetalle('${{d._id}}')">REVISAR</button></div>`;
                    else if(tipo === 'visitas') html += `<div class="card-mini"><b>${{d.pv}}</b><br><small>${{d.fecha}}</small></div>`;
                    else if(tipo === 'puntos') html += `<div class="card-mini"><b>${{d['Punto de Venta']}}</b><br><small>BMB: ${{d.BMB}}</small><button class="btn-g btn-outline" onclick="editar('${{tipo}}','${{d._id}}')">EDITAR</button></div>`;
                    else if(tipo === 'usuarios') html += `<div class="card-mini"><b>${{d.nombre_completo}}</b><br><small>${{d.rol}}</small><button class="btn-g btn-outline" onclick="editar('${{tipo}}','${{d._id}}')">EDITAR</button></div>`;
                }});
                grid.innerHTML = html;
            }}
            ...
            async function verDetalle(id) {{
                ...
                document.getElementById('m_body').innerHTML = `<h3>Validar</h3><img src="${{d.f_bmb}}" style="width:100%"><img src="${{d.f_fachada}}" style="width:100%; margin-top:5px;"><div style="display:flex; gap:5px;"><button class="btn-g btn-primary" onclick="finalizar('${{id}}','aprobar')">Aceptar</button><button class="btn-g btn-danger" onclick="finalizar('${{id}}','rechazar')">Rechazar</button></div>`;
            }}
            ...
            async function editar(tipo, id) {{
                ...
                const d = await r.json();
                let inputs = '<h3>Editar</h3><form id="ef">';
                for(let k in d) if(k!='_id' && !k.startsWith('f_')) inputs += `<label>\${{k}}</label><input name="\${{k}}" value="\${{d[k]}}">`;
                inputs += '</form><button class="btn-g btn-primary" onclick="guardar(\''+tipo+'\',\''+id+'\')">Guardar</button>';
                document.getElementById('m_body').innerHTML = inputs + '<button class="btn-g btn-outline" onclick="closeModal()">Cancelar</button>';
            }}
            ...
            function filtrar() {{
                ...
                document.querySelectorAll('.card-mini').forEach(c => c.style.display = c.innerText.toLowerCase().includes(val) ? 'block' : 'none');
            }}
        ...
        <div class="container" style="max-width:400px;">
            <div class="card-mini">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <input type="text" name="bmb" placeholder="BMB Detectado" required>
                    ...
                    <button class="btn-g btn-primary">Enviar</button>
                </form>
                <a href="/logout" class="btn-g btn-danger">Salir</a>
            </div>
        </div>
    </body></html>
    """)
