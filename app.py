...
<style>
    ...
    .card-mini { background: white; border-radius: 12px; padding: 15px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); border: 1px solid #efefef; position: relative; }
    ...
    .search-box input { width: 100%; padding: 12px 18px; border-radius: 25px; border: 1px solid #ddd; outline: none; }
...
def index():
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="search-box" id="search_container" style="display:none;"><input type="text" id="buscador" placeholder="🔍 Buscar..." onkeyup="filtrar()"></div>
            <div id="grid_data" class="grid-cards"></div>
        </div>
        <div id="m_global" class="modal"><div class="modal-content" id="m_body"></div></div>
        <script>
            let datosActuales = []; let tipoActual = ''; const miRol = "{rol}";
            ...
            async function verDetalleValidar(id, conBotones) {{
                ...
                let html = `<h3>Detalle</h3><p>${{d.pv}}<br>BMB Propuesto: ${{d.bmb_propuesto}}</p>
    ...
    <body onload="navigator.geolocation.getCurrentPosition(p=>document.getElementById('gps').value=p.coords.latitude+','+p.coords.longitude)">
        <div class="container" style="max-width:400px;">
            <div class="card-mini">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <label>Fotos</label><input type="file" name="f1" accept="image/*" capture="camera"><input type="file" name="f2" accept="image/*" capture="camera">
        ...
        <script>
            async function buscarPV(val) {{
                ...
                // AQUÍ SE RESTAURA EL AUTOCOMPLETADO DEL BMB
...
def exportar_reportes():
    if 'user_id' not in session or session.get('role') != 'admin': return "Acceso denegado", 403
    ...
    return Response(si.getvalue(), mimetype="text/csv", headers={"Content-disposition": f"attachment; filename=reportes.csv"})
...
def login():
    ...
    return render_template_string("<html><head>"+CSS_GERENCIAL+"</head><body style='display:flex; justify-content:center; align-items:center; height:100vh;'><div class='card-mini' style='width:300px;'><h3>Nestlé BI</h3><form method='POST'><input type='text' name='u' placeholder='Usuario'><input type='password' name='p' placeholder='Clave'><button class='btn-g btn-primary' style='width:100%'>Entrar</button></form></div></body></html>")
