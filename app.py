...
app = Flask(__name__)
app.secret_key = "nestle_bi_ultra_safe_2026"
...
<body>
    <div class="login-card">
        ...
        <p style="color:gray; font-size:14px;">Ingrese su número de cédula</p>
        <form action="/login" method="POST">
            <input type="password" name="cedula" placeholder="••••••••" required>
...
<body>
    ...
    <script>
        ...
        async function enviarVisita() {
            ...
            if(!document.getElementById('f_pv').value || f1.length < 100 || f2.length < 100) return alert("Faltan datos");
...
def login():
    if request.method == 'POST':
        ...
        # Buscamos en la columna 'pasworod' de la colección 'usuarios'
        user = usuarios_col.find_one({"pasworod": cedula})
        if user:
            session['user'] = cedula
            return redirect('/')
        else:
...
def api_buscar():
    ...
    # Consulta específica: si es número busca en BMB, si no en Punto de Venta
