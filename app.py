...
def conectar_google():
    try:
        ...
        # Ruta estándar para archivos de Environment Groups en Render
        ruta_creds = "/etc/secrets/creds.json"
        
        # Verificación de existencia del archivo
        if not os.path.exists(ruta_creds):
            return f"Archivo no encontrado en {ruta_creds}. Verifica que el Environment Group esté vinculado."
...
def index():
    ...
    if isinstance(hoja, str):
        return f"<h2 style='color:red'>Estado: Error de Configuración</h2><p>{hoja}</p>"
    
    try:
        registros = hoja.get_all_records()
    except Exception as e:
        return f"<h2>Error al leer la hoja</h2><p>{e}</p>"
    ...
    <head>
        ...
        <style>
            ...
            .btn { background: #0056a0; color: white; padding: 10px 15px; text-decoration: none; border-radius: 4px; }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>Visitas POC Nestlé</h1>
            <a href="/formulario" class="btn">+ Registrar Nueva Visita</a>
            <table>
                <tr>
...
def formulario():
    ...
    <div style="max-width:400px; margin:auto; background:white; padding:25px; border-radius:10px; font-family:sans-serif;">
        ...
        <form method="POST">
            ...
            <input type="text" name="mes" placeholder="Mes" required style="width:100%; margin-bottom:10px; padding:10px;">
