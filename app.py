...
app = Flask(__name__)
app.secret_key = "nestle_bi_ultra_2026_v2"
...
# --- CSS MEJORADO (CON AJUSTE DE BOTONES Y BARRA DE CARGA) ---
CSS_BI = """
<style>
    ...
    /* BOTONES CORREGIDOS PARA QUE NO SE SALGAN */
    .btn { width: 100%; padding: 12px; border-radius: 10px; font-weight: 700; cursor: pointer; border: none; transition: 0.2s; text-decoration: none; display: inline-block; font-size: 14px; box-sizing: border-box; margin-top: 10px; }
...
def index():
    ...
    <body>
        ...
        <div class="main-content">
            <div class="header-bar"><button class="menu-toggle" onclick="toggleMenu()">☰</button><h2 style="margin:0;">Registros</h2></div>
        ...
        <script>
            ...
            async function subirCsv() {{
                ...
                let width = 0;
                let interval = setInterval(() => {{ if(width >= 90) clearInterval(interval); else {{ width += 10; bar.style.width = width + '%'; }} }}, 200);
                ...
                try {{
                    ...
                    document.getElementById('statusMsg').innerText = "❌ Error al cargar el archivo.";
                }} finally {{
...
def carga():
    ...
    if f:
        try:
            ...
            reader = csv.DictReader(io.StringIO(content), dialect=csv.Sniffer().sniff(content[:1024]))
