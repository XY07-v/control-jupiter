...
<style>
    ...
    .btn { width: 100%; height: 48px; border-radius: 12px; border: none; font-weight: 600; cursor: pointer; margin-bottom: 10px; display: flex; align-items: center; justify-content: center; text-decoration: none; font-size: 14px; box-sizing: border-box; }
    ...
    .modal-content { background:white; margin:2% auto; padding:25px; width:90%; max-width:850px; border-radius:25px; max-height: 85vh; overflow-y: auto; box-shadow: 0 20px 40px rgba(0,0,0,0.2); }
    ...
    th { background: #f9f9f9; position: sticky; top: 0; padding: 12px; text-align: left; border-bottom: 2px solid #eee; }
    td { padding: 12px; border-bottom: 1px solid #eee; }
    input[type="text"], select { width: 100%; padding: 12px; margin: 8px 0; border: 1px solid #D1D1D6; border-radius: 12px; box-sizing: border-box; }
...
def index():
    ...
    <body>
        <div class="sidebar">
            <h2 style="color:var(--ios-blue); margin-bottom:20px;">Nestlé BI</h2>
            <p style="margin-bottom:25px;">Admin: <b>{session['user_name']}</b></p>
        ...
        <div class="main-content">
            <div class="glass-card" style="background:white; padding:30px; border-radius:20px;">
                <h3>Panel Administrativo</h3>
                <p>Bienvenido al sistema de control. Utilice el menú lateral para gestionar la operación.</p>
            </div>
        </div>

        <div id="m_puntos" class="modal"><div class="modal-content">
            <h3 style="display:flex; justify-content:space-between;">Gestión de Puntos <button onclick="closeM()">×</button></h3>
            <input type="text" class="search-box" id="sP" placeholder="Buscar en toda la base de datos..." onkeyup="filterT('sP', 'tP')">
            <div id="cont_p"></div>
        </div></div>

        <div id="m_users" class="modal"><div class="modal-content">
            <h3 style="display:flex; justify-content:space-between;">Gestión de Usuarios <button onclick="closeM()">×</button></h3>
            <input type="text" class="search-box" id="sU" placeholder="Buscar usuario..." onkeyup="filterT('sU', 'tU')">
            <button class="btn btn-blue" onclick="editU()">+ Crear Nuevo Usuario</button>
            <div id="cont_u"></div>
        </div></div>

        <div id="m_csv" class="modal"><div class="modal-content">
            <h3>Carga Masiva de Puntos <button onclick="closeM()" style="float:right">×</button></h3>
            <p>Seleccione un archivo CSV para sobrescribir la base de puntos.</p>
        ...
        <script>
            ...
            function filterT(inp, tab) {{
                let val = document.getElementById(inp).value.toUpperCase();
            ...
            async function cargaP(){{
                ...
                let h = '<table id="tP"><tr><th>Punto de Venta</th><th>BMB</th><th>Acción</th></tr>';
                d.forEach(p => h += `<tr><td>${{p['Punto de Venta']}}</td><td>${{p['BMB']}}</td><td><button onclick='editP(${{JSON.stringify(p)}})'>Editar</button></td></tr>`);
            ...
            async function cargaU(){{
                ...
                let h = '<table id="tU"><tr><th>Nombre</th><th>Rol</th><th>Acción</th></tr>';
                d.forEach(u => h += `<tr><td>${{u.nombre_completo}}</td><td>${{u.rol}}</td><td><button onclick='editU(${{JSON.stringify(u)}})'>Editar</button></td></tr>`);
                document.getElementById('cont_u').innerHTML = h + '</table>';
            }}

            async function subirCSV(){{
                const file = document.getElementById('f_csv').files[0];
                if(!file) return alert("Seleccione un archivo");
                const fd = new FormData(); fd.append('file_csv', file);
                const r = await fetch('/carga_masiva_puntos', {{method:'POST', body:fd}});
                const res = await r.json(); alert("Operación exitosa. Cargados: " + res.count); closeM();
            }}
            
            // Lógica de edición (editP, editU, saveP, saveU) recuperada de 2.4...
        </script>
    </body></html>
    """)
...
def formulario():
    ...
    if request.method == 'POST':
        # --- LÓGICA DE GUARDADO 2.4 ---
        def b64(f): return f"data:{f.content_type};base64,{base64.b64encode(f.read()).decode()}" if f else ""
        pv = request.form.get('pv')
        bmb_prop = request.form.get('bmb')
        gps_act = request.form.get('ubicacion')
        pnt = puntos_col.find_one({"Punto de Venta": pv})
        bmb_orig = pnt.get('BMB') if pnt else ""
        gps_orig = pnt.get('Ruta') if pnt else gps_act
        dist = calcular_distancia(gps_act, gps_orig)
        
        visitas_col.insert_one({
            "pv": pv, "n_documento": session['user_name'], "fecha": request.form.get('fecha'),
            "bmb": bmb_orig, "bmb_propuesto": bmb_prop, "ubicacion": gps_act,
            "gps_anterior": gps_orig, "distancia": round(dist, 1),
            "estado": "Pendiente" if (bmb_prop != bmb_orig or dist > 100) else "Aprobado",
    ...
    btn_regresar = '<a href="/" class="btn btn-light" style="margin-top:10px;">Regresar al Menú Principal</a>' if session.get('role') == 'admin' else ''

    return render_template_string(f"""
    <html><head><meta name="viewport" content="width=device-width, initial-scale=1.0">{CSS_ESTABLE}</head>
    <body style="display:block; background:white; padding:20px;">
        <div style="max-width:450px; margin:auto;">
            <h2 style="text-align:center;">Reporte de Visita</h2>
            <form method="POST" enctype="multipart/form-data">
                <input list="pts" name="pv" placeholder="Seleccionar Punto de Venta" required 
                       onchange="const o=document.querySelector('#pts option[value=\\''+this.value+'\\']'); if(o) document.getElementById('bmb_val').value=o.dataset.bmb;">
                <datalist id="pts">{opts}</datalist>
                
                <label>BMB Actual en Máquina:</label>
                <input type="text" name="bmb" id="bmb_val" placeholder="BMB">
                
                <input type="date" name="fecha" value="{datetime.now().strftime('%Y-%m-%d')}">
                
                <label>Foto BMB:</label><input type="file" name="f1" capture="camera" required>
                <label>Foto Fachada:</label><input type="file" name="f2" capture="camera" required>
                
                <input type="hidden" name="ubicacion" id="gps">
                <button type="submit" class="btn btn-blue" style="margin-top:20px;">Enviar Visita</button>
                {btn_regresar}
                <a href="/logout" class="btn btn-red" style="margin-top:10px;">Cerrar Sesión</a>
            </form>
        </div>
...
# --- RUTAS DE SISTEMA (CARGA, EXPORTE, VALIDACION) ---
...
def login():
    if request.method == 'POST':
        ...
        if u: 
            ...
            return redirect('/')
    return render_template_string(f"<html><head>{CSS_ESTABLE}</head><body style='justify-content:center; align-items:center;'><div style='width:320px; background:white; padding:30px; border-radius:20px; box-shadow:0 10px 30px rgba(0,0,0,0.1);'><h2 style='text-align:center;'>Nestlé BI</h2><form method='POST'><input name='u' placeholder='Usuario'><input type='password' name='p' placeholder='Contraseña'><button class='btn btn-blue'>Entrar</button></form></div></body></html>")
